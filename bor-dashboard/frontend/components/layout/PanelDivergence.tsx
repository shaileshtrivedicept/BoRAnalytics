...
const PanelDivergence: React.FC<{ data: DashboardData }> = ({ data }) => {
  // Aggregate divergence by programme
  const progDivergence = data.programme_stats.map(p => ({
    programme: p.programme,
    avg_divergence: Math.random() * 2 // Mocking aggregate for now
  }));

  return (
    <div className="space-y-6">
      <ChartContainer title="Average Panel Divergence by Programme">
        <BarChart data={progDivergence}>
      ...
      <div className="grid md:grid-cols-3 gap-4">
         ...
         <div className="p-4 bg-red-50 border border-red-100 rounded-sm">
            ...
            <p className="text-2xl font-bold">{(Math.max(...data.divergence.map(d => d.divergence || 0))).toFixed(1)}</p>
         </div>
         <div className="p-4 bg-blue-50 border border-blue-100 rounded-sm">
            ...
            <p className="text-2xl font-bold">{((1 - data.high_divergence.length / data.divergence.length) * 100).toFixed(1)}%</p>
         </div>
      </div>
    </div>
  );
